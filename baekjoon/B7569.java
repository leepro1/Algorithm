...
public class B7569 {
	public static void main(String[] args) throws IOException {
		...
		int[] dx = { -1, 1, 0, 0, 0, 0 }; // 상하좌우위아래 검사를 위한 배열
		int[] dy = { 0, 0, -1, 1, 0, 0 }; // 상하좌우위아래 검사를 위한 배열
		int[] dz = { 0, 0, 0, 0, -1, 1 }; // 상하좌우위아래 검사를 위한 배열
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < 6; i++) {
				...
				if (nextX < 0 || nextY < 0 || nextZ < 0 || nextX >= N || nextY >= M || nextZ >= H)
					continue;
				if (visited[nextZ][nextX][nextY] == 0) {
					visited[nextZ][nextX][nextY] = visited[temp[0]][temp[1]][temp[2]] + 1;
		...
		for (int i = 0; i < H; i++) {
			for (int j = 0; j < N; j++) {
				for (int k = 0; k < M; k++) {
					if (visited[i][j][k] == 0) { // 안익은 토마토가 있으면 0으로 break
						cnt = 0;
						break Loop;
					}
