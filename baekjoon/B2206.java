...
public class B2206 {
	public static void main(String[] args) throws IOException {
		...
		queue.offer(new int[] { 0, 0, 0 }); // 2번 index가 -1이면 벽을 부술 수 없다, 0이면 부술 수 있다.
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (nextX < 0 || nextY < 0 || nextX >= N || nextY >= M)
				...
				else if (map[nextX][nextY] == 0) { // 벽을 만났을 때
					if (temp[2] == 0) {// 벽을 부술 기회가 있을 때
					...
					map[nextX][nextY] = map[temp[0]][temp[1]] + 1;
					queue.offer(new int[] { nextX, nextY, temp[2] });
				}
			}
		}
		System.out.println(-1);
